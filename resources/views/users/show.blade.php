    ...
    <div class="col-6">
        <div class="card mb-2">
            <div class="card-header">
                <form id="deleteProductForm" action="{{ route('users.destroy', [$product->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <h5>
                        {{ $product->title }}
            ...
            <div class="card-body">
                <a href="{{ route('users.edit', [$product->id]) }}" class="btn btn-warning btn-sm float-right">Изменить</a>
